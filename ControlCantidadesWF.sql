...
declare @cFlujo int
	select @cFlujo = count(*) 
	from Workflow.Flujo f where f.IdFlujo = 1409001 
	print '    Workflow.Flujo REGISTROS CREADOS:  '+ CAST(@cFlujo AS CHAR(5)) 
	print '------------------------------------------------------------------------------------------'
	
	declare @cFlujoNodo int, @mig_1 int
	select @cFlujoNodo = count(*) 
	from Workflow.FlujoNodo where IdFlujo = 1409001
	select @mig_1 = count(*) 
	...
	select *--@cFlujoNodoPredecesor = count(*) 
	from Workflow.FlujoNodoPredecesor a where a.IdFlujo = 1409001
	...
	select @cConcepto = count(*) 
	from Workflow.Concepto c where c.IdConcepto in ('FLUJO_INI', 'GRUPO_BENEF', 'ID_TRAMITE', 'OFICINA_INI', 'ID_TRAMITE_MIG')
	print '    Workflow.Concepto REGISTROS CREADOS:  '+ CAST(@cConcepto AS CHAR(5)) 
	print '------------------------------------------------------------------------------------------'

	declare @cTipoTramiteConcepto int
	select @cTipoTramiteConcepto = count(*) 
	from Workflow.TipoTramiteConcepto c where c.IdConcepto in ('FLUJO_INI', 'GRUPO_BENEF', 'ID_TRAMITE', 'OFICINA_INI', 'ID_TRAMITE_MIG')
	...
	select @cSolicitudTramite = count(*) 
	from Workflow.SolicitudTramite 
	select @mig_2 = count(*) 
	from dbo.M_TRAMITES_ESTADOIN a where a.TipoTram = 'CC_CADQ' and a.flag not in (1,5,6)
	...
	select *--@cSolicitudTramiteConcepto = count(*) 
	from Workflow.SolicitudTramiteConcepto s where s.IdConcepto in ('FLUJO_INI', 'GRUPO_BENEF', 'ID_TRAMITE', 'OFICINA_INI', 'ID_TRAMITE_MIG')
	print '    Workflow.SolicitudTramiteConcepto REGISTROS CREADOS:  '+ CAST(@cSolicitudTramiteConcepto AS CHAR(5)) 
	print '------------------------------------------------------------------------------------------'
	
	declare @cInstancia int 
	select @cInstancia = count(*) 
	from Workflow.Instancia
	...
	select @cOficinaArea = count(*) 
	from Seguridad.OficinaArea a 
	print '    Seguridad.OficinaArea REGISTROS CREADOS:  '+ CAST(@cOficinaArea AS CHAR(5)) 
	print '------------------------------------------------------------------------------------------'

	declare @cRol int
	select @cRol = count(*) 
	from Seguridad.Rol r where r.Descripcion = 'Migrador Workflow'
